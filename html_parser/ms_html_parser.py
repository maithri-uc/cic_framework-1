...
class MSParseHtml(ParseHtml, RegexPatterns):
    ...
    def __init__(self):
        ...
        self.tag_type_dict: dict = {'head1': r'TITLE \d', 'ul': r'§ \d+-\d+-\d+', 'head2': r'^CHAPTER \d\.',
                              'head4': 'RESEARCH REFERENCES','head3': r'§ \d+-\d+-\d+',
                              'ol_p': r'^\(\d\)', 'junk1': '^Annotations$', 'normalp': '^Editor\'s note',
                              'article': r'^Article \d$|^Part \d$'}

        self.h4_head: list = [' Editor’s Notes —', 'Cross References —', 'NOTES TO DECISIONS','JUDICIAL DECISIONS','RESEARCH REFERENCES']
    ...
    def generate_class_name_dict(self):
        super(MSParseHtml, self).generate_class_name_dict()

    def replace_tags_titles(self):
        self.section_pattern = re.compile(r'^§*? (?P<id>\d+-\d+-\d+)', re.I)

        self.h2_order: list = ["title", 'chapter', 'article', 'part','']

        super(MSParseHtml, self).replace_tags_titles()

        num_p_tag = None
        for p_tag in self.soup.find_all("p"):
            if p_tag.get("class") in [[self.tag_type_dict["head4"]]]:
    ...
    def add_anchor_tags(self):
        super(MSParseHtml, self).add_anchor_tags()
    ...
    def create_analysis_nav_tag(self):
        ...
        print("Judicial Decisions nav created")
    ...
    def add_cite(self):
        ...
        for tag in self.soup.findAll(lambda tag: re.search(r"§+\s(\W+)?\d+-\w+-\d+(\.\d+)?"
            ...
            super(MSParseHtml, self).add_cite(tag)


ParseHtml_obj = MSParseHtml()
